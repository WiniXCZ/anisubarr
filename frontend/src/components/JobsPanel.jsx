          ...
          <div className="mt-0.5">
            ...
            {canExpand && (
              ...
              >
                ...
                {expanded ? t('job_less') : t('job_more')}
              </button>
            )}
          </div>
        )}
...
function LimitSelect({ value, onChange }) {
  ...
  return (
    <div className="relative flex items-center">
      ...
      >
        {LIMIT_OPTIONS.map((n) => (
          <option key={n} value={n}>{n} {t('job_records_suffix')}</option>
        ))}
      </select>
      ...
      >
        ...
        <div className="px-4 py-2 border-t border-border flex-shrink-0">
          <p className="text-xs text-muted text-center">
            {sorted.length > 0
              ? `${sorted.length} ${t('job_footer_of_last')} ${limit} ${t('job_records_suffix')}`
              : t('job_polling_note')}
          </p>
        </div>
      </aside>
