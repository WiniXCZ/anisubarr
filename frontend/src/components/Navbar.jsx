...
const NAV_ITEMS = [
  { to: "/",          icon: ICONS.dash,     labelKey: "nav_dashboard" },
  { to: "/library",   icon: ICONS.library,  labelKey: "nav_library" },
  { to: "/discover",  icon: ICONS.discover, labelKey: "nav_discover" },
  { to: "/calendar",  icon: ICONS.cal,      labelKey: "nav_calendar" },
  { to: "/requests",  icon: ICONS.req,      labelKey: "nav_requests" },
  { to: "/files",     icon: ICONS.files,    labelKey: "nav_files" },
  { to: "/users",     icon: ICONS.users,    labelKey: "nav_users" },
  { to: "/settings",  icon: ICONS.sett,     labelKey: "nav_settings" },
];
      ...
      <>
        ...
        {menuOpen && (
          <div style={{
            ...
            {[...NAV_ITEMS, { to: "/subtitles", icon: ICONS.subs, labelKey: "nav_subtitles" }].map(({ to, icon, labelKey }) => {
    ...
    <>
      <header style={{
        ...
        <nav style={{ flex: 1, display: "flex", justifyContent: "center", gap: 4, flexWrap: "wrap" }}>
          {NAV_ITEMS.map(({ to, icon, labelKey }) => navBtn(to, icon, t(labelKey)))}
        </nav>

        {/* Ctrl+K search hint */}
        <button
          onClick={() => document.dispatchEvent(new KeyboardEvent("keydown", { key: "k", ctrlKey: true, bubbles: true }))}
          title={t("nav_search_hint")}
          style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          ...
          <button
            onClick={logout} title={t("nav_logout")}
            style={{
              padding: "6px 7px", borderRadius: 8, border: `1px solid ${T.border}`,
