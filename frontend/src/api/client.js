...
export const scheduleBulkDownload      = (episodeIds, runAtIso) =>
  api.post("/subtitles/schedule-bulk-download", { episode_ids: episodeIds, run_at: runAtIso });
...
export const seerrReport          = (body)      => api.post("/seerr/report", body);
...
export const syncLibrary        = (id)    => api.post(`/libraries/${id}/sync`);
...
// ── AI Diagnostics ────────────────────────
export const getDiagnosticsContext = (logLines = 150) =>
  api.get("/ai-diagnostics/context", { params: { log_lines: logLines } });
export const analyzeDiagnostics    = (logLines = 150) =>
  api.post("/ai-diagnostics/analyze", null, { params: { log_lines: logLines }, timeout: 120_000 });
...
export const pauseDownload      = (hash)     => api.post(`/downloads/queue/${hash}/pause`);
export const resumeDownload     = (hash)     => api.post(`/downloads/queue/${hash}/resume`);
export const cancelDownload     = (hash, deleteFiles = false) =>
  api.delete(`/downloads/queue/${hash}`, { params: { delete_files: deleteFiles } });
