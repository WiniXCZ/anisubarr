...
// Status mapping for API status codes. `tKey` points at an i18n key
// (see locales/*.yaml, "status_*") — statusMeta(status, t) translates via
// that when a t() function is passed; callers that don't pass one yet fall
// back to the hardcoded Czech `label` below (being migrated page by page).
export const STATUS_META = {
  // AniList style
  RELEASING:        { label:'Vysílá se', tKey:'status_airing',   colorKey:'statusAiring' },
  FINISHED:         { label:'Dokončeno',  tKey:'status_finished', colorKey:'statusDone' },
  NOT_YET_RELEASED: { label:'Chystá se',  tKey:'status_upcoming', colorKey:'statusUpcoming' },
  CANCELLED:        { label:'Skončilo',   tKey:'status_ended',    colorKey:'statusEnded' },
  HIATUS:           { label:'Pauza',      tKey:'status_hiatus',   colorKey:'statusUpcoming' },
  // Sonarr style (Title case)
  Continuing:       { label:'Vysílá se', tKey:'status_airing',   colorKey:'statusAiring' },
  Ended:            { label:'Skončilo',   tKey:'status_ended',    colorKey:'statusEnded' },
  Upcoming:         { label:'Chystá se',  tKey:'status_upcoming', colorKey:'statusUpcoming' },
  Deleted:          { label:'Smazáno',   tKey:'status_deleted',  colorKey:'statusEnded' },
  // Sonarr style (lowercase — actual API values)
  // "ended" = series concluded (= completed/green), "deleted" = removed from Sonarr (= red)
  continuing:       { label:'Vysílá se', tKey:'status_airing',   colorKey:'statusAiring' },
  ended:            { label:'Dokončeno', tKey:'status_finished', colorKey:'statusDone' },
  upcoming:         { label:'Chystá se', tKey:'status_upcoming', colorKey:'statusUpcoming' },
  deleted:          { label:'Smazáno',  tKey:'status_deleted',  colorKey:'statusEnded' },
};
...
export function statusMeta(status, t) {
  ...
  if (!m) return { label: status || '—', colorKey: 'statusEnded' };
  return { label: t ? t(m.tKey) : m.label, colorKey: m.colorKey };
}
export function statusColor(theme, status, t) {
  return theme[statusMeta(status, t).colorKey];
}
