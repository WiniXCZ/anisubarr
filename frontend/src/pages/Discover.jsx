        ...
        {hovered && (
          <div style={{
            ...
            <button
              ...
              title={inWatchlist ? t('disc_remove_watchlist_title') : t('disc_add_watchlist_title')}
              style={{
                width: 34, height: 34,
                borderRadius: 8,
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 520 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            >
              {inWatchlist ? t('disc_watchlist_btn_in') : t('disc_watchlist_btn')}
            </button>
          </div>
        </div>
      </div>
...
function CountdownBadge({ startDate }) {
  ...
  let label;
  if (days < 0) label = t('disc_countdown_soon');
  else if (days === 0) label = t('disc_countdown_today');
  else if (days === 1) label = t('disc_countdown_tomorrow');
  else label = `${t('disc_countdown_in_days_prefix')} ${days} ${t('disc_countdown_in_days_suffix')}`;
  return (
    <span style={{
...
function getStatusOptions(t) {
  return [
    { value: "", label: t('lib_filter_all') },
    { value: "RELEASING", label: t('status_airing') },
    { value: "FINISHED", label: t('status_finished') },
    { value: "NOT_YET_RELEASED", label: t('disc_status_not_yet_released') },
    { value: "CANCELLED", label: t('disc_status_cancelled') },
    { value: "HIATUS", label: t('disc_status_hiatus') },
  ];
}
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <select value={genre} onChange={e => setGenre(e.target.value)} style={selectStyle()}>
            <option value="">{t('disc_all_genres')}</option>
            {genres.map(g => <option key={g} value={g}>{g}</option>)}
          </select>
          <select value={year} onChange={e => setYear(e.target.value)} style={selectStyle()}>
            <option value="">{t('disc_all_years')}</option>
        ...
        <>
          ...
          {hasNext && (
            <div style={{ display: "flex", justifyContent: "center", marginTop: 16 }}>
              ...
              >
                {loading ? t('common_loading') : t('disc_load_more')}
              </button>
            </div>
          )}
        </>
  ...
  async function handleToggleWatchlist(anime) {
    ...
    if (watchlistIds.has(id)) {
      ...
      try {
        ...
        toast.success(t('disc_toast_removed'));
      } catch (e) {
        toast.error(`${t('disc_toast_error_prefix')}: ${e.message}`);
      }
    } else {
      // Add
      try {
        ...
        toast.success(t('disc_toast_added'));
      } catch (e) {
        // Backend error text is Czech regardless of UI language — this substring
        // check against the raw message is pre-existing, not part of i18n.
        if (e.message?.includes("409") || e.message?.includes("již")) {
          toast.info(t('disc_toast_already'));
        } else {
          toast.error(`${t('disc_toast_error_prefix')}: ${e.message}`);
        }
      }
