...
// Well-known folder labels. Most are literal Sonarr folder names (not UI
// chrome), except "complete" which is our own label — translated via t()
// when available, falling back to the Czech default otherwise.
...
function locationLabel(key, t) {
  if (key === 'complete' && t) return t('status_finished');
...
function getSortOptions(t) {
  return [
    { key: 'title_asc',      label: t('lib_sort_title_asc') },
    { key: 'title_desc',     label: t('lib_sort_title_desc') },
    { key: 'year_desc',      label: t('lib_sort_year_desc') },
    { key: 'year_asc',       label: t('lib_sort_year_asc') },
    { key: 'score_desc',     label: t('lib_sort_score_desc') },
    { key: 'pct_desc',       label: t('lib_sort_pct_desc') },
    { key: 'pct_asc',        label: t('lib_sort_pct_asc') },
    { key: 'missing_desc',   label: t('lib_sort_missing_desc') },
    { key: 'missing_asc',    label: t('lib_sort_missing_asc') },
    { key: 'added_desc',     label: t('lib_sort_added_desc') },
    { key: 'added_asc',      label: t('lib_sort_added_asc') },
  ];
}
    ...
    <div onClick={onOpen} style={{
      ...
      <div style={{ flex:'1 1 0', minWidth:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          ...
          {series.promoted && <StatusPill theme={T} color={T.statusDone} label={t('lib_published_badge')} size="sm" dot={false}/>}
        </div>
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:200,
      ...
      <div style={{ background:T.panel, border:`1px solid ${T.borderStrong}`, borderRadius:12,
        ...
        <div style={{ font:'700 15px "Space Grotesk"', color:T.text }}>{t('lib_import_modal_title')}</div>
        {isLoading && (
          <div style={{ font:'500 12px "Space Grotesk"', color:T.textDim }}>{t('common_loading')}</div>
        )}
        {error && (
          <div style={{ font:'500 12px "Space Grotesk"', color:T.statusEnded }}>
            {t('lib_import_modal_error')}
          </div>
        )}
        {data?.error && (
        ...
        {!isLoading && !error && !data?.error && (
          folders.length === 0 ? (
            <div style={{ font:'500 12px "Space Grotesk"', color:T.textDim }}>
              {t('lib_import_modal_all_matched')}
            </div>
          ) : (
            <>
              <div style={{ font:'500 11px "Space Grotesk"', color:T.textDim }}>
                {folders.length} {folders.length === 1 ? t('lib_import_folder_singular') : t('lib_import_folder_plural')} {t('lib_import_modal_desc_suffix')}
              </div>
        ...
        <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
          <button style={btnSub(T)} onClick={onClose}>{t('lib_close')}</button>
          <button style={btnPrimary(T)} onClick={() => { onClose(); navigate('/discover'); }}>{t('lib_open_discover')}</button>
        </div>
      </div>
    </div>
  );
...
function BulkActionBar({ selectedIds, seriesList, onClear }) {
  ...
  const publishMutation = useMutation({
    ...
    onError: (e) => alert(`${t('lib_publish_failed')}: ${e.response?.data?.detail || e.message}`),
  });
  const demoteMutation = useMutation({
    mutationFn: (ids) => Promise.all(ids.map(id => demoteSeries(id))),
    onSuccess: () => { invalidate(); onClear(); },
    onError: (e) => alert(`${t('lib_demote_failed')}: ${e.response?.data?.detail || e.message}`),
  });
  const downloadSubsMutation = useMutation({
    mutationFn: (ids) => downloadAllBulkSeries(ids),
    onSuccess: () => { invalidate(); onClear(); },
    onError: (e) => alert(`${t('lib_download_subs_failed')}: ${e.response?.data?.detail || e.message}`),
  });
  const deleteSubsMutation = useMutation({
    mutationFn: (ids) => deleteSubsBySeries(ids),
    onSuccess: () => { invalidate(); onClear(); },
    onError: (e) => alert(`${t('lib_delete_subs_failed')}: ${e.response?.data?.detail || e.message}`),
  });
  ...
  const handleDeleteSubs = () => {
    const seriesWord = count === 1 ? t('lib_series_singular') : t('lib_series_plural');
    if (!window.confirm(`${t('lib_delete_subs_confirm_prefix')} ${count} ${seriesWord}${t('lib_delete_subs_confirm_suffix')}`)) return;
    ...
    <>
      <div style={{
        ...
        <div style={{
          ...
          {!anyPromoted && (
            <button disabled={busy} style={btnStyle(true)} onClick={() => publishMutation.mutate(ids)}>{t('lib_publish')}</button>
          )}
          {anyPromoted && (
            <button disabled={busy} style={btnStyle(false)} onClick={() => demoteMutation.mutate(ids)}>{t('lib_unpublish')}</button>
          )}
          <button disabled={busy} style={btnStyle(false)} onClick={() => downloadSubsMutation.mutate(ids)}>{t('lib_download_subs_btn')}</button>
          <button disabled={busy} style={{ ...btnGhost(T), padding: isMobile ? '8px 12px' : '6px 12px',
            ...
            onClick={handleDeleteSubs}>{t('lib_delete_subs_btn')}</button>
          <div style={{ flex:1, minWidth:16 }}/>
          <button style={{ ...btnGhost(T), padding: isMobile ? '8px 12px' : '6px 10px',
            fontSize:12, flexShrink:0, whiteSpace:'nowrap' }}
            onClick={onClear}>{t('lib_clear_selection')}</button>
        </div>
      </div>
    </>
  );
...
export default function Library() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      {!isMobile && (
        <PageHeader theme={T} title={t('lib_title')}
          subtitle={`${seriesList.length} ${t('lib_titles_suffix')} · ${watchedEps}/${totalEps} ${t('lib_episodes_downloaded_suffix')}`}
          right={<>
            <button style={btnSub(T)} onClick={() => setShowImport(true)}>{t('lib_import')}</button>
            <button style={btnPrimary(T)} onClick={() => navigate('/discover')}>{t('lib_add_anime')}</button>
          </>}
        />
      )}
      ...
      <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', padding:pad,
        ...
        {isMobile ? (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2, 1fr)', gap:8 }}>
            {[
              { label:t('lib_stat_watching_mobile'), value:watching,                       sub:t('lib_sub_active_series'),     accent:T.statusAiring },
              { label:t('lib_stat_planning_mobile'), value:counts.upcoming,                sub:t('lib_sub_waiting_premiere'),  accent:T.statusUpcoming },
              { label:t('status_finished'),          value:counts.completed,               sub:t('lib_sub_finished_series'),   accent:T.statusDone },
              { label:t('files_tab_downloads'),      value:stats?.active_downloads ?? '—', sub:t('lib_sub_active_jobs'),       accent:T.accent },
            ].map(p => (
          ...
          <div style={{ display:'flex', gap:10 }}>
            <StatCard theme={T} label={t('lib_stat_watching')} value={watching}                       sub={t('lib_sub_active_series')}    accent={T.statusAiring}/>
            <StatCard theme={T} label={t('lib_stat_planning')} value={counts.upcoming}                sub={t('lib_sub_waiting_premiere')} accent={T.statusUpcoming}/>
            <StatCard theme={T} label={t('status_finished')}   value={counts.completed}               sub={t('lib_sub_finished_series')}  accent={T.statusDone}/>
            <StatCard theme={T} label={t('files_tab_downloads')} value={stats?.active_downloads ?? '—'} sub={t('lib_sub_active_jobs')}    accent={T.accent}/>
            <StatCard theme={T} label={t('lib_stat_on_disk')}  value={stats?.disk_used ?? '—'}        sub={stats?.disk_total ? `${t('lib_sub_of_prefix')} ${stats.disk_total}` : t('lib_sub_used')} accent={T.accent2}/>
          </div>
        )}
        ...
        {isMobile ? (
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            ...
            <div style={{ display:'flex', gap:6, overflowX:'auto', scrollbarWidth:'none', paddingBottom:2 }}>
              {[
                { label:t('lib_filter_all'),      key:'all',       count:counts.all },
                { label:t('status_airing'),        key:'airing',    count:counts.airing },
                { label:t('status_finished'),      key:'completed', count:counts.completed },
                { label:t('status_upcoming'),      key:'upcoming',  count:counts.upcoming },
                { label:t('status_ended'),         key:'ended',     count:counts.ended },
              ].map(f => (
              ...
              <div style={{ flexShrink:0 }}>
                <FilterPill theme={T} label={t('lib_filter_promoted')} active={showPromotedOnly}
            ...
            {Object.keys(locationCounts).length > 0 && (
              <div style={{ display:'flex', gap:6, overflowX:'auto', scrollbarWidth:'none', paddingBottom:2 }}>
                <div style={{ flexShrink:0 }}>
                  <FilterPill theme={T} label={`📁 ${t('lib_filter_all')}`} active={locationFilter==='all'}
                    count={seriesList.length} onClick={() => setLocationFilter('all')}/>
                </div>
                {Object.entries(locationCounts).map(([k, cnt]) => (
                  <div key={k} style={{ flexShrink:0 }}>
                    <FilterPill theme={T} label={`📁 ${locationLabel(k, t)}`} active={locationFilter===k}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            ...
            <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
              <FilterPill theme={T} label={t('lib_filter_all')} active={filter==='all'}       count={counts.all}       onClick={() => setFilter('all')}/>
              <FilterPill theme={T} label={t('status_airing')}  active={filter==='airing'}    count={counts.airing}    onClick={() => setFilter('airing')}/>
              <FilterPill theme={T} label={t('status_finished')} active={filter==='completed'} count={counts.completed} onClick={() => setFilter('completed')}/>
              <FilterPill theme={T} label={t('status_upcoming')} active={filter==='upcoming'}  count={counts.upcoming}  onClick={() => setFilter('upcoming')}/>
              <FilterPill theme={T} label={t('status_ended')}   active={filter==='ended'}     count={counts.ended}     onClick={() => setFilter('ended')}/>
              <FilterPill theme={T} label={t('lib_filter_promoted')} active={showPromotedOnly}     count={counts.promoted}  onClick={() => setShowPromotedOnly(v => !v)}/>
              <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
                {selectedIds.size > 0 && (
                  <div style={{ font:'600 11px JetBrains Mono', color:T.accent }}>
                    {selectedIds.size} {t('lib_selected_suffix')}
                  </div>
                )}
                <input
                  value={search} onChange={e => setSearch(e.target.value)}
                ...
                <div style={{ display:'flex', background:T.panel2, padding:2, borderRadius:8, border:`1px solid ${T.border}` }}>
                  {[['grid',t('lib_view_grid')],['list',t('lib_view_list')]].map(([v, l]) => (
            ...
            {Object.keys(locationCounts).length > 0 && (
              <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                ...
                <FilterPill theme={T} label={t('lib_all_folders')} active={locationFilter==='all'}
                  count={seriesList.length} onClick={() => setLocationFilter('all')}/>
                {Object.entries(locationCounts).map(([k, cnt]) => (
                  <FilterPill key={k} theme={T} label={locationLabel(k, t)} active={locationFilter===k}
                    count={cnt} onClick={() => setLocationFilter(k)}/>
                ))}
              </div>
            )}
        ...
        {hasMore && (
          <div style={{ display:'flex', justifyContent:'center', paddingBottom:8 }}>
            <button
              ...
              {t('lib_load_more')} ({filtered.length - visibleCount} {t('lib_remaining_suffix')})
            </button>
          </div>
        )}
      </div>
