    ...
    >
      ...
      <div className="min-w-0">
        {editing ? (
          ...
          <p className="text-sm text-text leading-snug whitespace-pre-wrap break-words">{cue.text || <em className="text-muted text-xs">{t('player_empty_cue')}</em>}</p>
        )}
      </div>
...
export default function SubtitleEditorPage() {
    ...
    onSuccess: (data) => {
      if (data?.content) {
        ...
        if (parsed.length > 0) {
          setCues(parsed);
          setDirty(true);
          setSaveMsg({ ok: true, text: `${t('subed_shift_preview_prefix')} ${shiftMs > 0 ? "+" : ""}${shiftMs} ${t('subed_shift_preview_suffix')}` });
          setTimeout(() => setSaveMsg(null), 5000);
        } else {
          setSaveMsg({ ok: false, text: t('subed_shift_empty_result') });
        }
      }
    },
    onError: (e) => setSaveMsg({ ok: false, text: `${t('subed_shift_error_prefix')} ${e?.response?.data?.detail || e.message}` }),
  });
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      setSaveMsg({ ok: true, text: t('subed_saved_to_disk') });
      setDirty(false);
      setTimeout(() => setSaveMsg(null), 4000);
    },
    onError: (e) => setSaveMsg({ ok: false, text: `${t('subed_save_error_prefix')} ${e?.response?.data?.detail || e.message}` }),
  });
    ...
    <div className="-mx-4 -my-6 flex flex-col" style={{ height: "calc(100dvh - 3.5rem)" }}>
      ...
      <div className="flex-shrink-0 border-b border-border bg-surface px-4 py-3 flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          >
            <option value="">{t('subed_select_anime')}</option>
            {allSeries.map(s => (
