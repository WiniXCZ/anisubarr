...
function formatDate(str, lang = "cs") {
  if (!str) return "—";
  try {
    return new Date(str).toLocaleDateString(lang, { day: "numeric", month: "short" });
  } catch { return str; }
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {t('dash_upcoming_title')}
      </div>
      <div style={{ overflowY: "auto", flex: 1 }}>
        {isLoading ? (
          <div style={{ padding: 20, color: T.textMute, fontSize: 13 }}>{t('common_loading')}</div>
        ) : !data?.length ? (
          <div style={{ padding: 20, color: T.textMute, fontSize: 13 }}>{t('dash_no_episodes_week')}</div>
        ) : (
          data.map((ep, i) => (
            <Link
              key={i}
...
function RecentPanel({ items }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ overflowY: "auto", flex: 1 }}>
        {!items?.length ? (
          <div style={{ padding: 20, color: T.textMute, fontSize: 13 }}>{t('dash_no_series')}</div>
        ) : (
          items.map((s, i) => (
            <Link
              key={s.id}
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 5, flexShrink: 0 }}>
                ...
                <span style={{ fontSize: 11, color: T.textMute }}>{formatDate(s.added_at, lang)}</span>
              </div>
            </Link>
          ))
        )}
...
function PendingRequestsPanel({ items }) {
  const { t, lang } = useLang();
  // Backend currently returns these status strings in Czech regardless of UI
  // language (Seerr status text is not itself localized server-side) — map
  // the known values to the current UI language for display; unknown values
  ...
  const statusLabel = (status) => {
    const map = {
      "Dostupné": t('dash_status_available'),
      "Schváleno": t('dash_status_approved'),
      "Zpracovává se": t('dash_status_processing'),
      "Odmítnuto": t('dash_status_declined'),
    };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {t('dash_requests_title')}
      </div>
      <div style={{ overflowY: "auto", flex: 1 }}>
        {!items?.length ? (
          <div style={{ padding: 20, color: T.textMute, fontSize: 13 }}>{t('dash_no_requests')}</div>
        ) : (
          items.map((r, i) => (
            <div
              key={i}
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: T.textMute, fontFamily: "JetBrains Mono, monospace" }}>
                  {formatDate(r.requested_at, lang)}
                </div>
              </div>
              <span style={{
...
function ServicePanel({ health }) {
  const t = useT();
  // Service names are product/brand names (Sonarr, Emby/Jellyfin, Seerr) —
  // deliberately not translated, same as elsewhere in the app.
  const services = [
          ...
          return (
            <div key={key} style={{
              ...
              <span style={{
                ...
                {svc == null ? "—" : ok ? t('dash_online') : t('dash_offline')}
              </span>
            </div>
          );
        })}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
        ...
        {loadingSum && (
          <span style={{ fontSize: 12, color: T.textMute }}>{t('common_loading')}</span>
        )}
      </div>
      ...
      <div className="slide-up" style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        <StatBox label={t('dash_stat_total_series')} value={summary?.stats?.total_series} accent={T.accent} />
        <StatBox label={t('dash_stat_promoted')} value={summary?.stats?.promoted} accent={T.statusDone} />
        <StatBox label={t('dash_stat_missing_cs')} value={summary?.stats?.missing_cs} accent={T.statusEnded} />
      </div>
    </div>
  );
}
