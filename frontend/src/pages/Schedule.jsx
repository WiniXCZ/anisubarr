...
import {
  getSeries, getDownloadsQueue, getDownloadsRecent, getAppSettings,
  pauseDownload, cancelDownload, downloadBest,
} from '../api/client';
...
export default function Schedule({ theme }) {
  ...
  const pauseMutation = useMutation({
    mutationFn: (hash) => pauseDownload(hash),
    onSuccess: () => { qc.invalidateQueries(['downloads-queue']); toast.success('Stahování pozastaveno'); },
    onError: () => toast.error('Nepodařilo se pozastavit stahování'),
  });
  const cancelMutation = useMutation({
    mutationFn: (hash) => cancelDownload(hash),
    onSuccess: () => { qc.invalidateQueries(['downloads-queue']); toast.success('Stahování zrušeno'); },
    onError: () => toast.error('Nepodařilo se zrušit stahování'),
  });
  const subMutation = useMutation({
    mutationFn: (episodeId) => downloadBest({ episode_id: episodeId, language: 'cs' }),
    onSuccess: () => { toast.success('Hledám titulky…'); },
    onError: (e) => toast.error(e.response?.data?.detail || 'Nepodařilo se stáhnout titulky'),
  });
                ...
                return (
                  <div key={f.id || i} style={{
                    ...
                    <div style={{display:'flex',gap:4,justifyContent:'flex-end'}}>
                      <button style={{...btnGhost(T),padding:'4px 7px',fontSize:11}}
                        disabled={!f.hash || pauseMutation.isPending}
                        onClick={() => f.hash && pauseMutation.mutate(f.hash)}
                        title="Pozastavit">‖</button>
                      <button style={{...btnGhost(T),padding:'4px 7px',fontSize:11}}
                        disabled={!f.hash || cancelMutation.isPending}
                        onClick={() => { if (f.hash && window.confirm(`Zrušit stahování "${f.title || f.name}"?`)) cancelMutation.mutate(f.hash); }}
                        title="Zrušit">✕</button>
                    </div>
                  </div>
                );
                    ...
                    return (
                      <div key={j} onClick={() => navigate(`/series/${s.id || it.series_id || it.seriesId || ''}`)} style={{
                        ...
                        <div style={{display:'flex',gap:5}}>
                          <button style={{...btnGhost(T),padding:'4px 8px',fontSize:11}}
                            onClick={(e) => { e.stopPropagation(); navigate(`/series/${s.id || it.series_id || it.seriesId || ''}`); }}>
                            Otevřít
                          </button>
                          <button style={{...btnGhost(T),padding:'4px 8px',fontSize:11}}
                            disabled={subMutation.isPending}
                            onClick={(e) => { e.stopPropagation(); if (it.id) subMutation.mutate(it.id); }}>
                            Titulky
                          </button>
                        </div>
                      </div>
              ...
              {recent.slice(0, 10).map((f, i) => (
                <div key={f.id || i} style={{
                  ...
                  <div style={{display:'flex',gap:5,justifyContent:'flex-end'}}>
                    <button style={{...btnGhost(T),padding:'4px 8px',fontSize:11}}
                      disabled={!f.series_id}
                      onClick={() => f.series_id && navigate(`/series/${f.series_id}`)}>
                      Otevřít
                    </button>
                  </div>
                </div>
