...
function getSubStateMeta(t) {
  return {
    both:     { colorKey:'statusDone',     icon:'✓', label:t('sd_sub_both') },
    'cs-only':{ colorKey:'statusDone',     icon:'✓', label:t('sd_sub_cs') },
    jp:       { colorKey:'statusUpcoming', icon:'!', label:t('sd_sub_jp_only') },
    none:     { colorKey:'statusEnded',    icon:'×', label:t('sd_sub_missing') },
  };
}
    ...
    <div style={{
      ...
      <div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(130px, 1fr))', gap:'6px 16px' }}>
          <InfoCell label={t('sd_resolution_label')} value={ep.resolution || (ep.quality_resolution ? `${ep.quality_resolution}p` : null)} accent={T.accent} mono/>
          <InfoCell label={t('sd_codec_label')}       value={ep.video_codec} accent={T.accent2} mono/>
          ...
          <InfoCell label={t('sd_profile_label')}     value={ep.quality_name} mono/>
        </div>
      </div>
      ...
      <div style={{ borderTop:`1px solid ${T.border}`, paddingTop:10 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(130px, 1fr))', gap:'6px 16px' }}>
          <InfoCell label={t('sd_codec_label')}     value={ep.audio_codec} accent={T.accent2} mono/>
          <InfoCell label={t('sd_channels_label')}  value={aCh} mono/>
          <InfoCell label="Bitrate"     value={aBit} mono/>
          <InfoCell label={t('sd_languages_label')} value={ep.audio_languages} mono/>
        </div>
      </div>
      ...
      <div style={{ borderTop:`1px solid ${T.border}`, paddingTop:10 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(130px, 1fr))', gap:'6px 16px' }}>
          <InfoCell label={t('sd_size_label')}     value={sizeGB} accent={T.accent} mono/>
          <InfoCell label={t('sd_status_label')}   value={ep.has_file ? `✓ ${t('sd_file_downloaded')}` : `✗ ${t('sd_file_missing')}`} mono/>
          <InfoCell label={t('sd_subs_word')}      value={`${sm.icon} ${sm.label}`} mono/>
          <InfoCell label={t('sd_subs_source_label')} value={ep.subtitle_source || (ep.has_cs_sub ? t('sd_local_source') : null)} mono/>
        </div>
        {filename && (
          <div style={{ marginTop:8, font:'500 11px JetBrains Mono', color:T.textMute, wordBreak:'break-all' }}>
            <span style={{ color:T.textDim }}>{t('sd_file_colon')} </span>{filename}
          </div>
        )}
        {folder && (
      ...
      >
        ...
        <div style={{display:'flex', gap:4, flexShrink:0}} onClick={e => e.stopPropagation()}>
          {(subSt === 'both' || subSt === 'cs-only') && (
            <button onClick={onOpenSubs} style={{...btnGhost(T),padding:'4px 9px',fontSize:11}}>{t('sd_subs_word')}</button>
          )}
          {subSt === 'jp' && (
            <button onClick={onOpenSubs} style={{...btnPrimary(T),padding:'4px 9px',fontSize:11}}>✦ CS</button>
          )}
          {subSt === 'none' && ep.has_file && (
            <button onClick={onOpenSubs} style={{...btnPrimary(T),padding:'4px 9px',fontSize:11}}>↓ {t('sd_subs_word')}</button>
          )}
          {subSt === 'none' && !ep.has_file && (
            <button onClick={onOpenSubs} style={{...btnGhost(T),padding:'4px 9px',fontSize:11}}>↓</button>
          )}
          {(ep.has_file || ep.has_cs_sub) && onEdit && (
            <button onClick={onEdit} style={{...btnGhost(T),padding:'4px 9px',fontSize:11}} title={t('sd_open_sub_editor_title')}>✎</button>
          )}
        </div>
      </div>
...
function ScheduleView({ episodes, series, isMobile }) {
  ...
  function formatDate(dateStr) {
    ...
    try {
      ...
      return d.toLocaleDateString(lang, { weekday:'short', day:'numeric', month:'long', year:'numeric' });
  ...
  for (const ep of withDate) {
    ...
    try {
      const d = new Date(dateStr);
      monthKey = d.toLocaleDateString(lang, { month:'long', year:'numeric' });
    } catch {}
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      ...
      {(airTime || series.network) && (
        <div style={{ background:T.panel, border:`1px solid ${T.border}`, borderRadius:10,
          ...
          {airTime && (
            <div>
              <div style={{ font:'500 10px JetBrains Mono', color:T.textMute, marginBottom:4 }}>{t('sd_air_time_label')}</div>
              <div style={{ font:'700 18px JetBrains Mono', color:T.accent }}>{airTime}</div>
            </div>
          )}
          {series.network && (
            <div>
              <div style={{ font:'500 10px JetBrains Mono', color:T.textMute, marginBottom:4 }}>{t('sd_network_label_upper')}</div>
              <div style={{ font:'600 14px "Space Grotesk"', color:T.text }}>{series.network}</div>
            </div>
          )}
          <div>
            <div style={{ font:'500 10px JetBrains Mono', color:T.textMute, marginBottom:4 }}>{t('sd_episodes_label_upper')}</div>
            <div style={{ font:'600 14px "Space Grotesk"', color:T.text }}>
              {episodes.filter(e => isAired(e.air_date_utc || e.air_date)).length} / {episodes.length} {t('sd_aired_suffix')}
            </div>
          </div>
        </div>
      )}
...
function InfoView({ series, isMobile, onTranslate, translateState, onFetchEnTitle, enTitleState }) {
  ...
  const meta = statusMeta(series.status, t);
  return (
    <div style={{display:'grid',gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr',gap:14}}>
      <div style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:10,padding:16}}>
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}>
          <div style={{font:'700 14px "Space Grotesk"',color:T.text}}>{t('sd_metadata_label')}</div>
          {onFetchEnTitle && (
            <button
              ...
              title={t('sd_fetch_en_title_title')}>
              {enTitleState === 'loading' ? '⏳…'
                : enTitleState === 'ok' ? '✓'
                : enTitleState === 'error' ? '✗'
                : `🌐 ${t('sd_fetch_en_title_btn')}`}
            </button>
          )}
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          {[
            [t('sd_original_title_label'), series.title_japanese || '—'],
            ['Romaji', series.title_romaji || series.title || '—'],
            [t('sd_network_label'), series.network || '—'],
            [t('sd_year_label'), year],
            [t('sd_tab_episodes'), `${series.episode_count || '?'} ${t('dash_episodes_suffix')}`],
            [t('sd_genres_label'), (series.genres || []).join(' · ') || '—'],
            [t('sd_rating_label'), series.average_score > 0 ? `★ ${Number(series.average_score).toFixed(1)} / 10` : '—'],
            [t('sd_status_label'), meta.label],
            [t('sd_location_label'), series.path ? series.path.split(/[\\/]/).filter(Boolean).slice(-2).join('/') : '—'],
          ].map(([k, v]) => (
      ...
      <div style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:10,padding:16}}>
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}>
          <div style={{font:'700 14px "Space Grotesk"',color:T.text}}>{t('sd_synopsis_label')}</div>
          {series.overview_cs && (
          ...
          {onTranslate && (
            <button
              ...
              {translateState === 'loading' ? '⏳…'
                ...
                : series.overview_cs ? `↺ ${t('sd_retranslate_desc_btn')}`
                : `🌐 ${t('sd_translate_desc_btn')}`}
            </button>
          )}
        </div>
        <div style={{font:'500 13px/1.6 "Space Grotesk"',color:T.textDim}}>
          {series.overview_cs || series.overview || series.synopsis || t('sd_no_description')}
        </div>
      </div>
    </div>
  );
...
function getLogEventMeta(t) {
  return {
    state_change:        { icon:'⇄', label:t('sd_log_state_change'),      color:'#c897ff' },
    subtitle_search:     { icon:'🔎', label:t('sd_log_subtitle_search'),  color:'#5dade2' },
    seerr_report:        { icon:'🐞', label:'Seerr report',     color:'#ed4245' },
    hiyori_check:        { icon:'🌐', label:t('sd_log_hiyori_check'),  color:'#22c55e' },
    damage_eval:         { icon:'⚠', label:t('sd_log_damage_eval'),    color:'#ffa651' },
    subtitle_confidence: { icon:'✓', label:'Subtitle confidence', color:'#57f287' },
    info:                { icon:'ℹ', label:'Info',              color:'#9aa0ab' },
  };
}

function formatLogDate(dateStr, lang) {
  if (!dateStr) return '—';
  try {
    const d = new Date(dateStr);
    return d.toLocaleString(lang, { day:'numeric', month:'numeric', year:'numeric', hour:'2-digit', minute:'2-digit' });
  } catch { return dateStr; }
}
    ...
    <div style={{display:'flex', flexDirection:'column', gap:14}}>
      ...
      <div style={{background:T.panel, border:`1px solid ${T.border}`, borderRadius:10, padding:16,
        ...
        <div style={{display:'flex', flexDirection:'column', gap:6}}>
          <div style={{display:'flex', alignItems:'center', gap:8}}>
            <div style={{font:'700 14px "Space Grotesk"', color:T.text}}>{t('sd_audit_status_label')}</div>
          ...
          <div style={{font:'500 11px JetBrains Mono', color:T.textMute}}>
            {series.audit_status_since ? `${t('sd_since_prefix')} ${formatLogDate(series.audit_status_since, lang)}` : ''}
            {auditStatus?.last_hiyori_check_at ? ` · Hiyori check: ${formatLogDate(auditStatus.last_hiyori_check_at, lang)}` : ''}
          </div>
        </div>
        <button
          disabled={checking}
          ...
          {checking ? `⏳ ${t('sd_checking_ellipsis')}` : `↺ ${t('sd_audit_recheck_btn')}`}
        </button>
      </div>
      ...
      <div style={{background:T.panel, border:`1px solid ${T.border}`, borderRadius:10, overflow:'hidden'}}>
        {isLoading ? (
          ...
          logEntries.map((e, i) => {
            const logEventMeta = getLogEventMeta(t);
            const meta = logEventMeta[e.event_type] || logEventMeta.info;
            return (
                ...
                <div style={{flex:1, minWidth:0, display:'flex', flexDirection:'column', gap:2}}>
                  <div style={{display:'flex', alignItems:'center', gap:8, flexWrap:'wrap'}}>
                    ...
                    <span style={{font:'500 11px JetBrains Mono', color:T.textMute}}>
                      {formatLogDate(e.created_at, lang)}
                    </span>
                  </div>
...
function SubSearchModal({ theme, episodeId, epLabel, onClose }) {
  ...
  async function doSearch() {
    ...
    try {
      ...
      setError(e?.response?.data?.detail || t('sd_search_error'));
      setResults([]);
    } finally {
      setLoading(false);
    }
  ...
  async function doDownload(sub) {
    ...
    try {
      ...
      const detail = e?.response?.data?.detail || e?.message || t('sd_download_error_fallback');
      ...
      <div style={{
        ...
        <div style={{ padding:'10px 18px', borderBottom:`1px solid ${theme.border}`,
          ...
          <button onClick={doSearch} disabled={loading}
            ...
            {loading ? `⏳ ${t('sd_searching_ellipsis')}` : `↻ ${t('sd_research_btn')}`}
          </button>
        ...
        <div style={{ flex:1, overflowY:'auto', minHeight:0 }}>
          {loading && (
            <div style={{ padding:32, textAlign:'center', color:theme.textDim,
              ...
              ⏳ {t('sd_searching_sources_prefix')} Hiyori, HnS, Kamui, GenSubs…
            </div>
          )}
          ...
          {!loading && (results || []).map((sub, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                {/* Download button */}
                {isDone ? (
                  <span style={{ font:'600 11px JetBrains Mono', color:theme.statusDone, flexShrink:0 }}>✓ {t('sd_downloaded_badge')}</span>
                ) : (
                  <button
                    ...
                    {isDownloading ? '⏳' : `↓ ${t('sd_download_btn')}`}
                  </button>
                )}
              </div>
            );
        ...
        {logs.length > 0 && (
          <div style={{ borderTop:`1px solid ${theme.border}` }}>
            <button onClick={() => setShowLog(v => !v)} style={{
              ...
              {showLog ? '▲' : '▼'} Log ({logs.length} {t('sd_messages_suffix')})
            </button>
            {showLog && (
...
function ScheduleEpModal({ theme, count, episodeIds, onClose, onScheduled }) {
  const t = useT();
  ...
  async function submit() {
    if (!date || !time) { setError(t('sd_pick_date_time')); return; }
    const runAtLocal = `${date}T${time}:00`;
    const runAtDate  = new Date(runAtLocal);
    if (isNaN(runAtDate.getTime()) || runAtDate <= new Date()) {
      setError(t('sd_time_must_be_future'));
      return;
    }
    setSending(true);
    setError('');
    try {
      await scheduleBulkDownload(episodeIds, runAtLocal);
      onScheduled?.(runAtDate);
      onClose();
    } catch (e) {
      setError(e.response?.data?.detail || t('sd_schedule_failed'));
    } finally {
      setSending(false);
    }
  }
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.65)', zIndex:9999,
      ...
      <div style={{ width:340, maxWidth:'100%', minWidth:0, background:theme.panel, border:`1px solid ${theme.borderStrong}`,
        ...
        <div style={{ font:'500 11px JetBrains Mono', color:theme.textDim }}>
          {t('sd_schedule_desc_prefix')} {count} {t('sd_schedule_desc_suffix')}
        </div>
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          <label style={{ font:'600 10px JetBrains Mono', color:theme.textMute, letterSpacing:'.06em' }}>{t('sd_date_label')}</label>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          <label style={{ font:'600 10px JetBrains Mono', color:theme.textMute, letterSpacing:'.06em' }}>{t('sd_time_label')}</label>
        ...
        {error && (
          <div style={{ font:'500 11px "Space Grotesk"', color:theme.statusEnded }}>⚠ {error}</div>
        )}
        <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
          <button style={{ ...btnGhost(theme), padding:'6px 12px', fontSize:12 }} onClick={onClose}>{t('common_cancel')}</button>
          <button disabled={sending} style={{ ...btnPrimary(theme), padding:'6px 14px', fontSize:12, opacity: sending ? 0.6 : 1 }}
            onClick={submit}>{sending ? t('common_saving') : t('common_confirm')}</button>
        </div>
      </div>
    </div>,
    document.body
...
function getIssueTypeOptions(t) {
  return [
    { value: 3, label: t('sd_issue_type_subtitles') },
    { value: 1, label: t('sd_issue_type_video') },
    { value: 2, label: t('sd_issue_type_audio') },
    { value: 4, label: t('sd_issue_type_other') },
  ];
}

function ReportIssueModal({ theme, seriesId, onClose, onReported }) {
  const t = useT();
  const [issueType, setIssueType] = useState(3);
  ...
  async function submit() {
    ...
    try {
      ...
      const res = await seerrReport(body);
      onReported?.(res?.data);
      onClose();
    } catch (err) {
      setError(err?.response?.data?.detail || t('sd_report_failed'));
    } finally {
      setSending(false);
    }
  }
  ...
  return createPortal(
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.65)', zIndex:9999,
      ...
      <div style={{ width:380, maxWidth:'100%', minWidth:0, background:theme.panel, border:`1px solid ${theme.borderStrong}`,
        ...
        <div style={{ font:'500 11px JetBrains Mono', color:theme.textDim, lineHeight:1.5 }}>
          {t('sd_report_issue_desc')}
        </div>

        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          <label style={{ font:'600 10px JetBrains Mono', color:theme.textMute, letterSpacing:'.06em' }}>{t('sd_type_label')}</label>
          <select value={issueType} onChange={e => setIssueType(Number(e.target.value))} style={{
            padding:'7px 10px', background:theme.panel2, color:theme.text,
            border:`1px solid ${theme.border}`, borderRadius:6, outline:'none',
            font:'500 12px "Space Grotesk"' }}>
            {getIssueTypeOptions(t).map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
        </div>

        <div style={{ display:'flex', gap:8 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6, flex:1 }}>
            <label style={{ font:'600 10px JetBrains Mono', color:theme.textMute, letterSpacing:'.06em' }}>{t('sd_season_label')} {t('req_modal_optional')}</label>
            <input type="number" min="0" value={season} onChange={e => setSeason(e.target.value)} style={{
              padding:'7px 10px', background:theme.panel2, color:theme.text,
              border:`1px solid ${theme.border}`, borderRadius:6, outline:'none',
              font:'500 12px "Space Grotesk"' }}/>
          </div>
          <div style={{ display:'flex', flexDirection:'column', gap:6, flex:1 }}>
            <label style={{ font:'600 10px JetBrains Mono', color:theme.textMute, letterSpacing:'.06em' }}>{t('sd_episode_label_upper')} {t('req_modal_optional')}</label>
            <input type="number" min="0" value={episode} onChange={e => setEpisode(e.target.value)} style={{
              padding:'7px 10px', background:theme.panel2, color:theme.text,
              border:`1px solid ${theme.border}`, borderRadius:6, outline:'none',
              font:'500 12px "Space Grotesk"' }}/>
          </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          <label style={{ font:'600 10px JetBrains Mono', color:theme.textMute, letterSpacing:'.06em' }}>{t('sd_message_label')}</label>
          <textarea value={message} onChange={e => setMessage(e.target.value)} rows={3}
            placeholder={t('sd_describe_problem_placeholder')} style={{
              padding:'7px 10px', background:theme.panel2, color:theme.text,
              border:`1px solid ${theme.border}`, borderRadius:6, outline:'none', resize:'vertical',
              font:'500 12px "Space Grotesk"' }}/>
        </div>

        {error && (
          <div style={{ padding:'8px 10px', background:`${theme.statusEnded}18`,
            color:theme.statusEnded, font:'500 11px "Space Grotesk"',
            borderRadius:6, border:`1px solid ${theme.statusEnded}44` }}>
            ⚠ {error}
          </div>
        )}
        ...
        <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
          <button style={{ ...btnGhost(theme), padding:'6px 12px', fontSize:12 }} onClick={onClose}>{t('common_cancel')}</button>
          <button disabled={sending} style={{ ...btnPrimary(theme), padding:'6px 14px', fontSize:12, opacity: sending ? 0.6 : 1 }}
            onClick={submit}>{sending ? t('sd_reporting_ellipsis') : t('sd_report_btn')}</button>
        </div>
      </div>
    </div>,
    document.body
    ...
    <>
      {showSearch && epIdNums.length > 0 && (
        <SubSearchModal
          ...
          epLabel={`${count} ${t('sd_selected_episodes_suffix')}`}
      ...
      {showSchedule && (
        <ScheduleEpModal
          ...
          onScheduled={(when) => toast.success(`${t('sd_scheduled_for_prefix')} ${when.toLocaleString(lang)}`)}
        />
      )}
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ font:'600 12px "Space Grotesk"', color:theme.accent, flexShrink:0, marginRight:4 }}>
            {count} {t('lib_selected_suffix')}
          </div>
          <button style={{ ...btnSub(theme), padding: isMobile ? '7px 10px' : '6px 12px',
            fontSize:12, flexShrink:0, whiteSpace:'nowrap' }}
            onClick={() => setShowSchedule(true)}>⏱ {!isMobile && t('sd_schedule_short')}</button>
          <button style={{ ...btnPrimary(theme), padding: isMobile ? '7px 10px' : '6px 12px',
            fontSize:12, flexShrink:0, whiteSpace:'nowrap' }}
            onClick={handleDownloadBest}>⬇ {t('sd_subs_word')}</button>
          <button style={{ ...btnSub(theme), padding: isMobile ? '7px 10px' : '6px 12px',
            fontSize:12, flexShrink:0, whiteSpace:'nowrap' }}
            onClick={() => setShowSearch(true)}>🔍 {!isMobile && t('sd_search_word')}</button>
          <button style={{ ...btnGhost(theme), padding: isMobile ? '7px 10px' : '6px 12px',
            fontSize:12, flexShrink:0, whiteSpace:'nowrap' }}
            onClick={handleDeleteSubs}>🗑 {!isMobile && t('common_delete')}</button>
          ...
          <button style={{ ...btnGhost(theme), padding: isMobile ? '7px 10px' : '6px 10px',
            ...
            ✕ {!isMobile && t('sd_clear_selection_full')}
          </button>
        </div>
      </div>
    </>
...
export default function SeriesDetail({ theme }) {
  ...
  const handleSubDownload = async (force = false) => {
    ...
    try {
      ...
      setSubDlState('ok');
      toast.success(`${t('sd_subs_search_started')} (${count} ${t('sd_episodes_queued_suffix')})`);
      setTimeout(() => setSubDlState(null), 3000);
    } catch (err) {
      setSubDlState('error');
      const detail = err?.response?.data?.detail;
      toast.error(detail || t('sd_subs_search_start_error'));
      setTimeout(() => setSubDlState(null), 3000);
    }
  };
  ...
  const handleTranslate = async (force = false) => {
    ...
    try {
      ...
      toast.success(t('sd_desc_translated'));
      setTimeout(() => setTranslateState(null), 3000);
    } catch (err) {
      setTranslateState('error');
      const detail = err?.response?.data?.detail;
      toast.error(detail || t('sd_translate_failed'));
      setTimeout(() => setTranslateState(null), 3000);
    }
  };
  ...
  const handleFetchEnTitle = async () => {
    ...
    try {
      ...
      toast.success(res.data.title_en
        ? `${t('sd_en_title_saved_prefix')} ${res.data.title_en}`
        : t('sd_en_title_not_found'));
      setTimeout(() => setEnTitleState(null), 3000);
    } catch (err) {
      setEnTitleState('error');
      toast.error(err?.response?.data?.detail || t('sd_anilist_not_found'));
      setTimeout(() => setEnTitleState(null), 3000);
    }
  };
  ...
  const handleFetchTmdb = async () => {
    ...
    try {
      ...
      qc.invalidateQueries(['series', id]);
      setTmdbState('ok');
      toast.success(t('sd_tmdb_data_loaded'));
      setTimeout(() => setTmdbState(null), 3000);
    } catch (err) {
      setTmdbState('error');
      toast.error(err?.response?.data?.detail || t('sd_tmdb_not_found'));
      setTimeout(() => setTmdbState(null), 3000);
    }
  };
  ...
  const handlePromotion = async (promote) => {
    ...
    try {
      ...
      setPromoState('ok');
      toast.success(promote ? t('sd_promote_started') : t('sd_demote_started'));
      setTimeout(() => {
        setPromoState(null);
        qc.invalidateQueries(['series', id]);
      }, 2500);
      ...
      setPromoState('error');
      toast.error(err?.response?.data?.detail || (promote ? t('sd_promote_failed') : t('sd_demote_failed')));
      setTimeout(() => setPromoState(null), 3000);
    }
  };

  const handleSyncSeries = async () => {
    if (!series?.sonarr_id) return;
    setSyncState('loading');
    try {
      await syncOne(series.sonarr_id);
      toast.success(t('sd_sync_started'));
      setTimeout(() => {
        setSyncState(null);
        qc.invalidateQueries(['series', id]);
        qc.invalidateQueries(['episodes', id]);
      }, 2500);
    } catch (err) {
      setSyncState('error');
      toast.error(err?.response?.data?.detail || t('sd_sync_failed'));
    ...
    <div style={{flex:1, minHeight:0, overflowY:'auto', WebkitOverflowScrolling:'touch'}}>
      ...
      {reportModalOpen && (
        <ReportIssueModal
          ...
          onReported={() => {
            toast.success(t('sd_issue_reported'));
            qc.invalidateQueries(['series', id]);
          }}
        />
      )}
        ...
        {isMobile ? (
          <div style={{position:'relative', padding:'12px 14px 16px', display:'flex', flexDirection:'column', gap:10}}>
            <button onClick={() => navigate('/')} style={{
              ...
              <NavIcon d={ICONS.back}/> {t('player_back')}
            </button>
            ...
            <div style={{display:'flex', gap:6, flexWrap:'wrap'}}>
              ...
              <button
                ...
                title={embyData?.url ? t('sd_open_in_emby') : t('sd_not_in_emby')}
              ...
              <div style={{display:'flex', gap:0}}>
                <button
                  ...
                  title={missingCount > 0 ? `${t('sd_dl_subs_mobile_title_prefix')} ${missingCount} ${t('sd_episodes_word')}` : t('sd_research_all_mobile')}
                  ...
                  {subDlState === 'loading' ? '⏳'
                    : subDlState === 'ok' ? `✓ ${t('sd_started_badge')}`
                    : subDlState === 'error' ? `✗ ${t('disc_toast_error_prefix')}`
                    : missingCount > 0 ? `↓ ${t('sd_subs_word')} (${missingCount})`
                    : `↓ ${t('sd_subs_word')} ✓`}
                </button>
                <button
                  disabled={subDlState === 'loading'}
                  onClick={() => handleSubDownload(true)}
                  title={t('sd_force_research_mobile')}
              ...
              <button
                disabled={syncState === 'loading' || !series.sonarr_id}
                onClick={handleSyncSeries}
                title={t('sd_sync_from_sonarr')}
                style={{...btnSub(T), padding:'9px 10px', fontSize:12, flexShrink:0,
                  background:'rgba(255,255,255,0.12)', color:'#fff',
                  border:'1px solid rgba(255,255,255,0.2)',
                  opacity: (syncState === 'loading' || !series.sonarr_id) ? 0.5 : 1}}>
                {syncState === 'loading' ? '⏳' : syncState === 'ok' ? '✓' : '↻'}
              </button>
              <button
                onClick={() => setReportModalOpen(true)}
                title={t('sd_report_issue_short')}
                style={{...btnSub(T), padding:'9px 10px', fontSize:12, flexShrink:0,
                  background:'rgba(237,66,69,0.2)', color:'#ff9b9b',
          ...
          <div style={{position:'relative', padding:'18px 24px 22px', display:'flex', gap:22}}>
            <div style={{position:'absolute', top:14, left:24}}>
              <button onClick={() => navigate('/')} style={{
                ...
                <NavIcon d={ICONS.back}/> {t('sd_back_to_library')}
              </button>
            </div>
            ...
            <div style={{flex:1,minWidth:0,marginTop:34,display:'flex',flexDirection:'column',gap:12,color:'#fff'}}>
              ...
              <div style={{display:'flex',gap:8,marginTop:6,flexWrap:'wrap'}}>
                <button
                  ...
                  ▶ {t('sd_continue_ep_prefix')} {downloaded + 1 <= total ? String(downloaded + 1).padStart(2,'0') : '—'}
                </button>
                <button
                  disabled={!embyData?.url}
                  onClick={() => embyData?.url && window.open(embyData.url, '_blank', 'noopener,noreferrer')}
                ...
                <div style={{display:'flex', gap:0}}>
                  <button
                    ...
                    title={missingCount > 0
                      ? `${t('sd_dl_subs_desktop_title_prefix')} ${missingCount} ${t('sd_dl_subs_desktop_title_suffix')}`
                      : t('sd_all_have_subs_desktop')}
                    style={{...btnSub(T), background:'rgba(255,255,255,0.12)', color:'#fff',
                      border:'1px solid rgba(255,255,255,0.2)',
                      borderRight: 'none',
                      ...
                      opacity: subDlState === 'loading' ? 0.5 : 1}}>
                    {subDlState === 'loading' ? `⏳ ${t('sd_searching_ellipsis')}`
                      : subDlState === 'ok' ? `✓ ${t('sd_started_badge')}`
                      : subDlState === 'error' ? `✗ ${t('disc_toast_error_prefix')}`
                      : missingCount > 0 ? `↓ ${t('sd_subs_word')} (${missingCount})` : `↓ ${t('sd_subs_word')} ✓`}
                  </button>
                  <button
                    disabled={subDlState === 'loading'}
                    onClick={() => handleSubDownload(true)}
                    title={t('sd_force_research_desktop')}
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      toast.success(`${t('sd_langcheck_done_prefix')} ${fixed} ${t('sd_subs_genitive_suffix')}`);
                      setTimeout(() => setLangCheckState(null), 3000);
                    } catch {
                      setLangCheckState('error');
                      toast.error(t('sd_langcheck_failed'));
                      setTimeout(() => setLangCheckState(null), 3000);
                    }
                  }}
                  style={{...btnSub(T), background:'rgba(255,255,255,0.12)', color:'#fff',
                  ...
                  {langCheckState === 'loading' ? `⏳ ${t('sd_checking_ellipsis')}`
                    : langCheckState === 'ok' ? `✓ ${t('files_state_completed')}`
                    : langCheckState === 'error' ? `✗ ${t('disc_toast_error_prefix')}`
                    : `🔍 ${t('sd_check_subs_lang_btn')}`}
                </button>
                <button
                  disabled={translateState === 'loading' || !aiReady}
                  title={!aiReady ? t('sd_ai_not_configured_title') : undefined}
                  ...
                  {!aiReady ? `🤖 ${t('sd_ai_not_set_badge')}`
                    : translateState === 'loading' ? `⏳ ${t('sd_translating_ellipsis')}`
                    : translateState === 'ok' ? `✓ ${t('sd_translated_badge')}`
                    : translateState === 'error' ? `✗ ${t('disc_toast_error_prefix')}`
                    : series.overview_cs ? `↺ ${t('sd_retranslate_desc_btn')}`
                    : `🌐 ${t('sd_translate_desc_btn')}`}
                </button>
                <button
                ...
                <button
                  ...
                  {syncState === 'loading' ? '⏳ Sync…'
                    : syncState === 'ok' ? '✓ Sync'
                    : syncState === 'error' ? '✗ Sync'
                    : `↻ ${t('sd_sync_from_sonarr')}`}
                </button>
                <button
                  onClick={() => setReportModalOpen(true)}
                  title={t('sd_report_issue_full_title')}
                  style={{...btnSub(T),background:'rgba(237,66,69,0.2)',color:'#ff9b9b',
                    border:'1px solid rgba(237,66,69,0.4)',
                    padding:'7px 12px', fontSize:13, borderRadius:8}}>
                  🐞 {t('sd_report_issue_btn')}
                </button>
                {series.anilist_id && (
                  <a href={`https://anilist.co/anime/${series.anilist_id}`} target="_blank" rel="noopener noreferrer"
                    style={{...btnSub(T),background:'rgba(255,255,255,0.12)',color:'#fff',
                      border:'1px solid rgba(255,255,255,0.2)',
                      padding:'7px 12px', fontSize:13, borderRadius:8,
                ...
                {series && (series.promoted ? (
                  <button
                    disabled={promoState === 'loading'}
                    onClick={() => handlePromotion(false)}
                    title={t('sd_demote_full_title')}
                    style={{...btnSub(T), padding:'7px 14px', fontSize:13, borderRadius:8,
                      background:'rgba(237,66,69,0.25)', color:'#ff8080',
                      border:'1px solid rgba(237,66,69,0.45)',
                      opacity: promoState === 'loading' ? 0.6 : 1}}>
                    {promoState === 'loading' ? `⏳ ${t('sd_demoting')}`
                      : promoState === 'ok' ? `✓ ${t('sd_started_badge')}`
                      : promoState === 'error' ? `✗ ${t('disc_toast_error_prefix')}`
                      : `↓ ${t('sd_demote_short')}`}
                  </button>
                ) : (
                  <button
                    disabled={promoState === 'loading'}
                    onClick={() => handlePromotion(true)}
                    title={t('sd_promote_full_title')}
                    style={{...btnSub(T), padding:'7px 14px', fontSize:13, borderRadius:8,
                      background:'rgba(87,242,135,0.2)', color:'#57f287',
                      border:'1px solid rgba(87,242,135,0.4)',
                      opacity: promoState === 'loading' ? 0.6 : 1}}>
                    {promoState === 'loading' ? `⏳ ${t('sd_promoting')}`
                      : promoState === 'ok' ? `✓ ${t('sd_started_badge')}`
                      : promoState === 'error' ? `✗ ${t('disc_toast_error_prefix')}`
                      : `↑ ${t('sd_promote_short')}`}
                  </button>
      ...
      <div style={{display:'flex',gap:2,padding:'0 24px',borderBottom:`1px solid ${T.border}`,
        ...
        {[
          ['eps',      `${t('sd_tab_episodes')} · ${total}`],
          ['schedule', t('sd_tab_schedule')],
          ['info',     'Info'],
          ['log',      'Log'],
        ].map(([k, l]) => (
      ...
      <div style={{
        ...
        {activeTab === 'eps' && <>
          <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}>
            <FilterPill theme={T} label={t('lib_filter_all')}         active={epFilter==='all'}          count={regularEps.length} onClick={() => setEpFilter('all')}/>
            <FilterPill theme={T} label={t('sd_unwatched_filter')}    active={epFilter==='unwatched'}    count={regularEps.filter(e => !e.watched).length} onClick={() => setEpFilter('unwatched')}/>
            <FilterPill theme={T} label={t('sd_missing_subs_filter')} active={epFilter==='missing-subs'} count={missingCount} onClick={() => setEpFilter('missing-subs')}/>
            {specialEps.length > 0 && (
              <FilterPill theme={T} label={t('sd_specials_label')} active={epFilter==='specials'} count={specialEps.length} onClick={() => setEpFilter('specials')}/>
            )}
            <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:8}}>
              {selectedEpIds.size > 0
                ? <button style={{...btnGhost(T),padding:'4px 9px',fontSize:11}}
                    ...
                    {t('sd_select_all_prefix')} ({filteredEps.length})
                  </button>
                : null}
              <div style={{font:'500 11px JetBrains Mono',color:T.textMute}}>
                {filteredEps.length} {t('lib_sub_of_prefix')} {episodes.length}
              </div>
            </div>
          </div>
