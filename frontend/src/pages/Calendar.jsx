...
export default function Calendar({ theme }) {
  ...
  const t = useT();
  const MONTH_NAMES = t('cal_months');
  const DAY_NAMES = t('cal_days');
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
      <PageHeader theme={T} title={t('cal_title')}
        subtitle={t('cal_subtitle')}
        right={<>
                  ...
                  <div key={di} style={{
                    ...
                    {day && (
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline'}}>
                        ...
                        {drops.length > 0 && (
                          <div style={{font:'600 9px JetBrains Mono',color:T.textMute}}>{drops.length} {t('cal_ep_suffix')}</div>
                        )}
                      </div>
                    )}
