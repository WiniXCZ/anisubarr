...
def _qbit_queue(db) -> list[dict]:
    ...
    try:
        ...
        # Login — qBittorrent 4.1+ validates Origin/Referer (CSRF), 5.x also
        # checks Sec-Fetch-Site. Without these headers the login silently
        # returns HTTP 403 and this function returns an empty queue.
        ...
        if login_resp.text.strip() not in ("Ok.", ""):
            return []
        # Get torrents
