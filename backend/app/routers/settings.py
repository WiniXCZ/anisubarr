...
EDITABLE_KEYS: set[str] = {
    ...
    # ── Media access mode ─────────────────────────────────────────────────
    "media_access_mode",   # str — "local" (root/volume-mounted directory) or "smb" (network share)
...
_SECRET_KEYS: set[str] = {
    ...
    "discord_webhook_url", "smtp_password",
...
async def test_connection(
    ...
    if service == "sonarr":
        ...
        return await _test_smtp(body, db)
...
async def _test_smtp(body: dict, db: Session) -> dict:
    """Send a real test email to the address in body['to'] (or the SMTP from-address)."""
    to_addr = body.get("to") or _get_setting(db, "smtp_from_email") or ""
    if not to_addr:
        return {"connected": False, "reason": "not_configured"}
    try:
        from ..services.mailer import send_test_mail
        send_test_mail(to_addr, db=db)
        return {"connected": True}
    except Exception as exc:
        return {"connected": False, "reason": str(exc)}
