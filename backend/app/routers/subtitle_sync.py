...
def _find_cs_sub(ep: Episode) -> str | None:
    ...
    for sub in ep.subtitles:
        ...
        try:
            # If already a local/UNC path, resolve() would just log a noisy
            # "does not start with prefix" warning and return it unchanged.
            resolved = sub.file_path if sub.file_path.startswith("\\\\") else path_resolver.resolve(sub.file_path)
...
def _run_ffsubsync(video_path: str, cs_sub: str) -> dict:
    """
    Fallback subtitle sync using ffsubsync (audio-VAD based, no reference-track
    extraction needed). Used automatically by _run_alass() when alass fails or
    produces an unusable result — a different engine sometimes succeeds where
    alass can't (e.g. no embedded reference track AND unusual audio codec).
    """
    if not shutil.which("ffsubsync"):
        return {"status": "error", "message": "ffsubsync not installed (fallback unavailable)"}
    ...
    try:
        proc = subprocess.run(
            ["ffsubsync", video_path, "-i", cs_sub, "-o", tmp_path],
        ...
        if proc.returncode != 0:
            ...
            return {"status": "error", "message": f"ffsubsync exited with code {proc.returncode}: {err}"}

        out_size = os.path.getsize(tmp_path) if os.path.exists(tmp_path) else 0
        if out_size < 50:
            try: os.unlink(tmp_path)
            except OSError: pass
            return {"status": "error", "message": f"ffsubsync produced an empty output ({out_size} bytes)"}
        ...
        return {
            "status":        "ok",
            "message":       f"Synchronizováno (fallback ffsubsync) {os.path.basename(cs_sub)}",
        ...
        return {"status": "error", "message": "ffsubsync timed out (>300 s)"}
    except Exception as exc:
        try: os.unlink(tmp_path)
        except OSError: pass
...
def _alass_or_fallback(local_video: str, cs_sub: str, alass_error: dict) -> dict:
    """
    Called whenever the alass attempt failed. Tries ffsubsync as a second
    engine; if that also fails, returns a combined error message so both
    failure reasons are visible in the job log.
    """
    log.warning("alass failed (%s) — trying ffsubsync fallback", alass_error.get("message"))
    fb = _run_ffsubsync(local_video, cs_sub)
    if fb.get("status") == "ok":
        fb["message"] = f"[alass selhal → použit ffsubsync] {fb['message']}"
        return fb
    return {
        "status": "error",
        "message": (
            f"alass: {alass_error.get('message', '?')} | "
            f"ffsubsync fallback: {fb.get('message', '?')}"
        ),
    }
...
def _run_alass(ep: Episode) -> dict:
    ...
    ffsubsync fallback if alass fails or produces an unusable result.
    ...
    if ref_tmp:
        ...
        if ref_tmp:
            ...
            # alass can't even start without a reference — but ffsubsync does its
            # own audio decode internally, so it's still worth trying here.
            return _alass_or_fallback(local_video, cs_sub, no_ref_error)
    reference = ref_tmp
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            return _alass_or_fallback(local_video, cs_sub, alass_err)
        ...
        if out_size < 50 or out_entries == 0:
            ...
            return _alass_or_fallback(local_video, cs_sub, empty_err)
        ...
        if not entries_ok:
            ...
            return _alass_or_fallback(local_video, cs_sub, lost_err)
        ...
        try:
            ...
            pass
        return _alass_or_fallback(local_video, cs_sub, {"status": "error", "message": "alass timed out (>180 s)"})
        ...
        try:
            ...
            pass
        return _alass_or_fallback(local_video, cs_sub, {"status": "error", "message": str(exc)})
