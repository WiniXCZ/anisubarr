...
def send_test_message(webhook_url: str) -> None:
    """
    Send a one-off test message to verify a webhook URL works.
    Called from the Settings "Otestovat" button (routers/settings.py::_test_discord).
    Raises on failure (bad URL, HTTP error) — the caller catches and reports it.
    """
    embed = {
        "title":       "🔔 Testovací zpráva",
        "description": "Pokud vidíš tuhle zprávu, webhook je správně nastavený.",
        "color":       _COLOR_INFO,
        "footer":      _build_footer("Test"),
        "timestamp":   _now_ts(),
    }
