...
# job_id -> run_id, only for job_ids currently running — lets callers check
# "is a job of this type already running" before starting a duplicate (e.g.
# the manual Sonarr sync button, or a webhook, firing while one is in flight).
_running_by_job_id: dict[str, str] = {}
...
def start_run(job_id: str, job_name: str) -> JobRun:
    ...
    with _lock:
        ...
        _running_by_job_id[job_id] = run.run_id
    _db_write(run)
    _db_prune()
    return run
...
def is_job_running(job_id: str) -> bool:
    """True if a run with this job_id is currently in progress."""
    with _lock:
...
def _unmark_running(run_id: str, job_id: str | None = None) -> None:
    """Remove run_id from all "currently running" tracking. Caller must hold _lock."""
    ...
    if job_id is not None:
        ...
        # Fallback: no job_id known (e.g. cancel_run_id callers) — scrub by value
        for jid, rid in list(_running_by_job_id.items()):
            if rid == run_id:
                _running_by_job_id.pop(jid, None)
...
def finish_run(run: JobRun, status: str, message: str = "") -> None:
    ...
    run.message     = message
    with _lock:
        _unmark_running(run.run_id, run.job_id)
    _db_update(run)
    ...
    try:
        ...
        try:
            ...
            if row and row.status == "running":
                ...
                db.commit()
                with _lock:
                    _unmark_running(run_id, row.job_id)
                return True
            return False
        finally:
            db.close()
