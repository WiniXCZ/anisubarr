...
def job_auto_backup():
    """Create a DB snapshot and prune old ones beyond the configured limit."""
    ...
    db = SessionLocal()
    try:
        keep_raw = read_setting("auto_backup_keep_count", db) or "14"
    finally:
        db.close()
    try:
    ...
    meta = backup_svc.create_backup(label="auto")
    deleted = backup_svc.prune_backups(keep)
    log.info("[scheduler] auto_backup → %s created, %d old backups pruned (keep=%d)",
              meta["filename"], deleted, keep)
...
JOB_REGISTRY: dict[str, dict] = {
    ...
    "auto_backup": {
        "fn":          job_auto_backup,
        "name":        "Automatická záloha",
        "description": "Vytvoří zálohu databáze a smaže staré nad rámec limitu (Nastavení → Systém → Záloha a obnova)",
        "interval":    "daily",
        "hour":        4,
        "minute":      0,
    },
...
def _wrap(job_id: str, fn: Callable):
    def wrapper():
        ...
        # Avoid overlapping runs of the same job. Two separate trackers exist:
        # `_running_jobs` here (scheduler-triggered runs only) and job_log's
        # own `_running_by_job_id` (also covers runs started outside the
        # scheduler, e.g. the manual "Sync" button posting straight to
        # /api/sync/sonarr). Check both so a manual sync in flight blocks a
        # scheduled run of the same job_id, and vice versa.
        with _running_lock:
            already = job_id in _running_jobs or job_log.is_job_running(job_id)
            if already:
